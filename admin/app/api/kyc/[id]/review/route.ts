  ...
  if (business?.owner_id) {
    ...
    if (owner?.email) {
      ...
      sendEmailViaResend({
        ...
        html:
          decision === 'approved'
            ? `<h2>¡Negocio verificado!</h2>${greeting}<p>${business.name} obtuvo la insignia de "verificado" en SOSmoto.</p><a href="${APP_URL}" style="display:inline-block;margin-top:16px;padding:12px 24px;background:#FF6B00;color:#fff;text-decoration:none;border-radius:8px;font-weight:bold;">Ver en SOSmoto</a>`
            : `<h2>Verificación rechazada</h2>${greeting}<p>Tu solicitud de verificación fue rechazada.</p>${adminNotes ? `<p><strong>Motivo:</strong> ${adminNotes}</p>` : ''}<p>Puedes corregir y volver a enviar tus documentos desde la app.</p><a href="${APP_URL}" style="display:inline-block;margin-top:16px;padding:12px 24px;background:#FF6B00;color:#fff;text-decoration:none;border-radius:8px;font-weight:bold;">Ver en SOSmoto</a>`,
