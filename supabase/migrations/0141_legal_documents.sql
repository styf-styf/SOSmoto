-- Términos y Política de Privacidad pasan de HTML estático a contenido
-- editable desde el admin. Cada publicación crea una fila nueva (version+1)
-- en vez de sobrescribir -- deja historial y "última actualización" sale
-- sola de published_at, no hay que tocarla a mano.
create table legal_documents (
  id uuid primary key default gen_random_uuid(),
  type text not null check (type in ('terms', 'privacy')),
  version int not null,
  content text not null, -- HTML de las secciones (h2/p/ul), sin el shell de la pagina
...
-- Lectura pública -- /terminos y /privacidad son páginas sin login, y la app
-- también necesita leerlas para el banner de "términos actualizados".
create policy legal_documents_select_public on legal_documents for select using (true);

-- Solo el admin puede publicar (los inserts los hace admin/, con service
-- role -- esta policy es la que aplicaría si algún día se hiciera desde el
-- cliente autenticado, hoy no aplica porque el admin panel usa service role).
create policy legal_documents_insert_admin on legal_documents for insert to authenticated with check (is_admin());

-- Cuándo el usuario vio/aceptó (implícito por seguir usando la app, o
-- explícito si toca "Entendido" en el banner) la versión más reciente
-- publicada de CUALQUIERA de los dos documentos -- un solo timestamp para
-- ambos, ya que se publican juntos desde la misma sección del admin.
alter table users add column legal_ack_at timestamptz;

-- Semilla: contenido actual de web/public/terminos.html y privacidad.html
-- como version 1 de cada uno, para no perder lo que ya se escribió.
...
<li>Debes tener capacidad legal para contratar según las leyes de Ecuador.</li>
...
<p>Los Negocios pueden solicitar una insignia de "verificado" enviando documentación (cédula/RUC, permisos, fotos del local). SOSmoto revisa y aprueba o rechaza estas solicitudes a su criterio; la insignia no constituye garantía de calidad del servicio del Negocio.</p>
...
<tr><td>Rol (cliente/negocio) y datos del negocio (nombre, dirección, horario, RUC/cédula para KYC)</td><td>Operar el perfil de negocio y verificarlo</td></tr>
...
<tr><td>Payphone</td><td>Procesar el pago del negocio hacia la plataforma (suscripción/publicidad)</td></tr>
