...
// sin publicaciones -- separada del perfil del negocio (BusinessProfileView)
...
export function NegocioCatalogoView({ businessId, hrefBase }: { businessId: string | undefined; hrefBase: string }) {
  ...
  const { data, loading, reload } = useCachedLoad<NegocioCatalogoData>(cacheKey, async () => {
    if (!businessId) return { business: null, services: [], products: [] };
    ...
    <ScrollView contentContainerStyle={styles.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={handleRefresh} colors={[colors.primary]} />}>
      ...
      {business.business_type === 'workshop' && (
        <Section title="Servicios">
          {services.length === 0 ? (
            ...
            <CatalogGrid items={services} hrefBase={`${hrefBase}/servicio`} />
          )}
        </Section>
      )}
      ...
      <Section title="Productos">
        {products.length === 0 ? (
          ...
          <CatalogGrid items={products} hrefBase={`${hrefBase}/producto`} />
        )}
      </Section>
    </ScrollView>
  );
