...
// Compartida entre cliente y negocio -- el cuerpo entero (carga, toggle,
// permisos del sistema) es idéntico; lo único que cambia por rol es qué
// categorías se muestran (el negocio además filtra "auxilio" si es tienda).
export function NotificationPrefsScreen({
  ...
  const load = useCallback(async () => {
    if (!profile) return;
    const [nextPrefs, nextOptions] = await Promise.all([
      getNotificationPrefs(profile.id),
      loadOptions(profile.id),
    ]);
    setPrefs(nextPrefs);
