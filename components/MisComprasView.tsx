...
// Compartida entre cliente y negocio (compras B2B) -- la única diferencia
// real de comportamiento es que el cliente puede cancelar un apartado
// propio y el negocio no (compra B2B, no tiene sentido "cancelar" del lado
// comprador de esa forma); allowCancel controla eso explícitamente en vez
// de duplicar todo el archivo.
...
function PurchaseCard({
  ...
  return (
    <Pressable style={styles.card} onPress={() => router.push(`${hrefBase}/producto/${purchase.productId}`)}>
      ...
      {allowCancel && (purchase.status === 'pending' || purchase.status === 'confirmed') && (
        <Button
